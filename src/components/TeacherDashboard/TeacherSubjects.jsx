...
export default function TeacherSubjects() {
  ...
  if (error) {
    message.error(error?.data?.message || "Error getting teacher's subjects");
  }
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 max-w-3xl text-xs sm:text-sm">
                Here are all the subjects you're currently teaching.
              </p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-8 sm:p-12 text-center border border-indigo-200">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              You currently don't have any assigned subjects. Please contact the administration if this is unexpected.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.map((subject, index) => (
              ...
              >
                <div className="p-8 flex flex-col items-center text-center space-y-4 flex-1">
                  ...
                  <div className="flex items-center justify-center px-4 py-2 bg-indigo-100 rounded-full text-sm sm:text-base text-indigo-800 font-medium mb-4">
                    Class {subject.classTitle} - Section {subject.section}
                  </div>
                </div>
              </div>
            ))}
