...
export default function ChangeStudentPassword() {
  ...
  if (!student) {
    message.error("No student data provided");
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 px-4">
        ...
        <button
          className="flex items-center gap-2 px-2 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all duration-200 shadow-lg hover:shadow-xl font-nunito text-sm"
        >
  ...
  const [changeStudentPassword, { isLoading: isSubmitting }] = useChangePasswordMutation();
  ...
  const validateFormData = () => {
    if (!formData.oldPassword || !formData.newPassword) {
      message.error("Both old password and new password are required");
      return false;
    }
    if (
  ...
  const handleSubmit = async (e) => {
    ...
    const messageKey = "changeStudentPassword";
          ...
          <div className="flex items-center gap-3 mb-1 md:mb-0">
            ...
            <div>
              ...
              <p className="text-sm md:text-base text-gray-500 font-nunito">
                Update password for {capitalizeName(student.name)}
              </p>
            </div>
          </div>
