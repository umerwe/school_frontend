...
export default function ChangeAdminPassword() {
  ...
  if (!user) {
    message.error("No admin data available");
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 px-4">
        <div className="text-red-600 mb-6 text-lg text-center font-nunito">
          No admin data found.
        </div>
        <button
  ...
  const handleSubmit = async (e) => {
    ...
    const messageKey = "changeAdminPassword";
          ...
          <div className="flex items-center gap-3 mb-1 md:mb-0">
            ...
            <div>
              ...
              <p className="text-sm md:text-base text-gray-500 font-nunito">
                Update password for {capitalizeName(user.instituteName)}
              </p>
            </div>
          </div>
          <button
            onClick={() => navigate("/admin-dashboard/profile")}
            className="flex text-[14px] sm:text-sm items-center justify-center gap-2 px-4 py-2 bg-gray-600 text-white hover:bg-gray-700 rounded-lg font-medium font-nunito transition-colors shadow-sm hover:shadow-md"
          >
