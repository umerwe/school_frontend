...
export default function ParentVouchers() {
  ...
  // Clear loading states when component mounts or when user returns from external page
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        // Clear all loading states when user returns to the page
        setLoadingVouchers({});
      }
    };

    const handleFocus = () => {
      // Clear all loading states when window gets focus
      setLoadingVouchers({});
    };
  ...
  const handlePayment = async (voucherId) => {
    ...
    try {
      ...
      // Set a timeout to clear the loading state in case something goes wrong
      ...
      // Clear the timeout since we're handling the response
      ...
      // The loading state will be cleared by the visibility/focus event listeners
          ...
          <div className="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-gray-800 font-medium font-nunito">
                This feature is currently in test mode. Use card number 4242 4242 4242 4242 for payments. Expiry date can be any future date, and CVV can be any 3 digits.
              </p>
            </div>
          </div>
